...
export class RpcWebSocketClient {
    public static config = {
        getWs(url: string, protocols?: string | string[]) {
    ...
    public async connect(url: string, protocols?: string | string[]) {
        if (!WebSocket) {
            throw new Error(`For server side usage you need to specify WebSocket creator function.`);
        }

        this.ws = this.config.getWs(url, protocols);
        await this.listen();
    }
